...
function install_snort {
	...
	sed -i 's|DEBIAN_SNORT_HOME_NET="192.168.0.0/16"|DEBIAN_SNORT_HOME_NET="10.10.10.20,192.168.1.23,172.16.30.25"|' /etc/snort/snort.debian.conf
	# Default to current interface so that it starts
	local SNORT_IFACES="$(route -n | grep -E ^0.0.0.0 | awk '{print $8}')"
